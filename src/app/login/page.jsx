  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res?.error) {
        ...
        router.push('/');
        router.refresh(); // Supaya status login di navbar terupdate
      }
    } catch (err) {
  ...
  return (
    <>
      ...
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'var(--bg-light)', padding: '2rem' }}>
        <div style={{ backgroundColor: 'white', padding: '2.5rem', borderRadius: '20px', boxShadow: '0 10px 25px rgba(0,0,0,0.05)', width: '100%', maxWidth: '450px' }}>
          
          <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
            <h1 style={{ fontSize: '2rem', color: 'var(--primary-dark)', marginBottom: '0.5rem' }}>Selamat Datang</h1>
            <p style={{ color: 'var(--text-muted)' }}>Silakan masuk untuk mengikuti lelang</p>
          </div>

          {error && (
            <div style={{ backgroundColor: '#fee2e2', color: '#991b1b', padding: '1rem', borderRadius: '10px', marginBottom: '1.5rem', fontSize: '0.9rem', textAlign: 'center' }}>
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                ...
                <a href="#" style={{ fontSize: '0.85rem', color: 'var(--primary-cyan)', textDecoration: 'none' }}>Lupa Password?</a>
              </div>
          ...
          <p style={{ textAlign: 'center', marginTop: '1.5rem', fontSize: '0.9rem', color: '#6b7280' }}>
            Belum punya akun? <Link href="/register" style={{ color: 'var(--primary-cyan)', fontWeight: '600', textDecoration: 'none' }}>Daftar di sini</Link>
          </p>
